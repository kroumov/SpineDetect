...
%         cal_params.ext_rate = 292.3;                                       % Somas needs an even higher extrusion rate
    ...
    case 'double'
        ...
        S_somas       = S_somas(:,l_buff+1:end);                           % Remove buffering period
    case 'voltage'
        cal_params   = check_cal_params([], 'voltage');
        [~, S_somas] = spk_calcium(S_times, cal_params.vPars);
        S_somas      = cell2mat(S_somas);                                  % Turn the traces back into a matrix
    otherwise
...
%         cal_params.ext_rate = 292.3/2;                                     %
    ...
    case 'voltage'
        cal_params  = check_cal_params([], 'voltage');
        [~, S_dend] = spk_calcium(S_times, cal_params.vPars);
        S_dend      = cell2mat(S_dend);                                    % Turn the traces back into a matrix
    otherwise
        error('Unknown dynamics type to simulate!')
end
else
...
%         cal_params.ext_rate = 292.3/4;                                     % 
    ...
    case 'voltage'
        cal_params  = check_cal_params([], 'voltage');
        [~, S_axon] = spk_calcium(S_times, cal_params.vPars);
        S_axon      = cell2mat(S_axon);                                    % Turn the traces back into a matrix
    otherwise
        error('Unknown dynamics type to simulate!')
end
else
...
if mod(floor(1./spike_opts.dt), 10) == 0                                   % For efficiency, get lowest common denomenator factors
    ...
    [r1, r2] = pickResampleVals(spike_opts.dt); 
end
...
function [r1, r2] = pickResampleVals(dt)

diff1 = abs((1./dt) - floor(1./dt));
diff2 = abs((100*dt) - floor(100*dt)); 

if diff2 < diff1
    r1 = 100;                                                              % Set up/down sampling rates for resampling
    r2 = floor(1./dt);
else
    r2 = 1;                                                                % Set up/down sampling rates for resampling
    r1 = floor(101*dt);
end
