...
elseif max(lambda(:)) == 0
    ...
    for kk = 1:N_gaps
        ...
        elseif numel(lambda) == (size(NeurProf, 3)+n_BGs)                  % Case where each profile and each background component has it's own lambda
            lambda2 = single(single(lambda(:))*single(ones(1,n_k)));       % Make lambda the same size as x_all expects to be
        elseif numel(lambda) == n_t*(size(NeurProf, 3)+n_BGs)              % Case where each profile and each background component has it's own lambda at every time
            lambda2 = single(reshape(lambda(:,(T_gaps(kk)+1):T_gaps(kk+1)),...
