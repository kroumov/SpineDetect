...
vol_sz_orig = vol_params.vol_sz;                                          % save; check_vol_params rounds vol_sz(3) to 10, breaks vcpx match
vol_params   = check_vol_params(vol_params);                               % Check volume parameters
vol_params.vol_sz = vol_sz_orig;                                          % restore actual slab size (thin slabs e.g. 15.36um)
...
vres   = vol_params.vres;                                                  % Get volume resolution XY (pixels per micron)
...
% Ensure vasc_sz(3) covers zB: indexing zA+1:zB requires vascpx(3)>=zB
...
if (isfield(vol_out,'neur_ves_all'))&&~isempty(vol_out.neur_ves_all)
  ...
  vcpx = [round(vol_params.vasc_sz(1)*vres), round(vol_params.vasc_sz(2)*vres), round(vol_params.vasc_sz(3)*vres_z)];  % anisotropic
  if(~all(nvpx==vcpx))
    if ~isempty(lp0) && exist('warn_append','file')
      warn_append(lp0, 'FALLBACK TMPvasc: neur_ves_all [%d,%d,%d] ~= vcpx [%d,%d,%d], creating TMPvasc\n', ...
        nvpx(1), nvpx(2), nvpx(3), vcpx(1), vcpx(2), vcpx(3));
    end
    TMPvasc = false(vcpx);
    if(all(nvpx<=vcpx))
      dpx = (vcpx-nvpx)/2;
...
volpx     = [vol_sz(1:2)*vres, vol_sz(3)*vres_z];                          % anisotropic
vascpx    = [vasc_sz(1:2)*vres, vasc_sz(3)*vres_z];                         % anisotropic
psfsz     = psf_params.psf_sz;
psfpx     = [psfsz(1:2)*vres, psfsz(3)*vres_z];                             % anisotropic
proppx    = round(psf_params.prop_sz*vres_z);                               % Z stride for groupzproject

zA   = round(vres_z*(vol_depth+vol_sz(3)/2)-psfpx(3)/2);                   % Z voxel index
zB   = round(vres_z*(vol_depth+vol_sz(3)/2)+psfpx(3)/2);
zAsz = [vasc_sz(1:2)*vres zA/proppx];
if((~isfield(psf_params,'propcrop'))||psf_params.propcrop)
  gb = 2*gaussianBeamSize(psf_params,psfsz(3)/2,3)+[vol_params.vol_sz(1:2) psfsz(3)];
  zBsz = [vres*gb(1), vres*gb(2), vres_z*gb(3)];                            % anisotropic
  if(zBsz(1)>zAsz(1))
    zBsz(1:2) = zAsz(1:2);
  end
else
...
if (isfield(psf_params,'taillength'))&&(~isempty(psf_params.taillength))
  zCsz = [vasc_sz(1:2)*vres psf_params.taillength*vres_z/proppx];           % anisotropic
else
  zCsz = [vasc_sz(1:2)*vres (vasc_sz(3)*vres_z-zB)/proppx];                 % anisotropic
end

% Clamp zA, zB to vessel volume Z extent (avoid index out of bounds)
if (isfield(vol_out,'neur_ves_all'))&&~isempty(vol_out.neur_ves_all)
  nvz = size(vol_out.neur_ves_all, 3);
  nv = size(vol_out.neur_ves_all);
  zAsz = [nv(1), nv(2), zA/proppx];                                         % match volume XY exactly
  zBsz(1:2) = zAsz(1:2);
  zCsz(1:2) = zAsz(1:2);
elseif ~isempty(TMPvasc)
  nvz = size(TMPvasc, 3);
  nv = size(TMPvasc);
  zAsz = [nv(1), nv(2), zA/proppx];
  zBsz(1:2) = zAsz(1:2);
  zCsz(1:2) = zAsz(1:2);
else
  nvz = round(vasc_sz(3)*vres_z);
end
zA = max(1, min(zA, nvz));
zB = max(zA+1, min(zB, nvz));
...
%       neur_ves_A(:,:,end) = neur_ves_A(:,:,end)*rem(zAsz(3),1);
    ...
    neur_ves_B = masked_3DGP_test(ceil(zBsz),scatter_sz,1,0,[],[],scatter_wt);
...
elseif(isempty(TMPvasc))
  ...
  % Extract TMPves; (1:zBsz(1)) can yield fewer elements than ceil(zBsz), so pad to match neur_ves_B
  zBsz_n1 = min(floor(zBsz(1)), vascpx(1));
  zBsz_n2 = min(floor(zBsz(2)), vascpx(2));
  TMPves = single(vol_out.neur_ves_all(floor((vascpx(1)-zBsz_n1)/2)+(1:zBsz_n1), ...
    floor((vascpx(2)-zBsz_n2)/2)+(1:zBsz_n2),zA+1:zB));
  % Pad TMPves to match neur_ves_B; third dim uses szB(3)
  szB = ceil(zBsz);
  TMPves_pad = zeros(szB(1), szB(2), szB(3), 'single');
  n1 = min(size(TMPves,1), szB(1)); n2 = min(size(TMPves,2), szB(2)); n3 = min(size(TMPves,3), szB(3));
  if n3 < szB(3)
    if exist('warn_append','file'), warn_append(lp0, 'PAD: TMPves z=%d < neur_ves_B z=%d, zero-padding %d layers (symmetric)\n', n3, szB(3), szB(3)-n3); else warning('simulate_optical_propagation:Z_PAD', 'PAD: TMPves z=%d < neur_ves_B z=%d', n3, szB(3)); end
  end
  if n1 < szB(1) || n2 < szB(2)
    if exist('warn_append','file'), warn_append(lp0, 'PAD: TMPves [%d,%d] < neur_ves_B [%d,%d] (symmetric)\n', n1, n2, szB(1), szB(2)); else warning('simulate_optical_propagation:XY_PAD', 'PAD: TMPves [%d,%d] < neur_ves_B [%d,%d]', n1, n2, szB(1), szB(2)); end
  end
  tx = floor((szB(1)-n1)/2); ty = floor((szB(2)-n2)/2); tz = floor((szB(3)-n3)/2);
  TMPves_pad(1+tx:tx+n1, 1+ty:ty+n2, 1+tz:tz+n3) = TMPves(1:n1, 1:n2, 1:n3);
  neur_ves_B = TMPves_pad + neur_ves_B.* ... 
    bsxfun(@minus,ones(size(TMPves_pad,1),size(TMPves_pad,2),'single'),TMPves_pad);

  TMPves = groupzproject(single(vol_out.neur_ves_all(:,:,zB+1:end)),proppx,'sum');
  nC = size(neur_ves_C,3); nT = size(TMPves,3);
  if nT > nC
    if exist('warn_append','file'), warn_append(lp0, 'CROP: neur_ves_C z=%d, TMPves z=%d, symmetric center crop\n', nC, nT); else warning('simulate_optical_propagation:Z_CROP', 'CROP'); end
    tz = floor((nT - nC)/2);
    TMPves_crop = TMPves(:,:,1+tz:tz+nC);
    neur_ves_C = TMPves_crop + neur_ves_C.* ... 
      bsxfun(@minus,ones(size(TMPves_crop,1),size(TMPves_crop,2),'single'),TMPves_crop/proppx);
  elseif nT == nC
    neur_ves_C = TMPves + neur_ves_C.* ...
      bsxfun(@minus,ones(size(TMPves,1),size(TMPves,2),'single'),TMPves/proppx);
  else
    if exist('warn_append','file'), warn_append(lp0, 'PAD: neur_ves_C z=%d, TMPves z=%d, zero-padding %d layers (symmetric)\n', nC, nT, nC-nT); else warning('simulate_optical_propagation:Z_PAD_C', 'PAD'); end
    tz = floor((nC - nT)/2);
    neur_ves_C(:,:,1+tz:tz+nT) = TMPves + neur_ves_C(:,:,1+tz:tz+nT).* ...
      bsxfun(@minus,ones(size(TMPves,1),size(TMPves,2),'single'),TMPves/proppx);    
  end
else
  ...
  % Extract TMPves; pad to match neur_ves_B
  zBsz_n1 = min(floor(zBsz(1)), size(TMPvasc,1));
  zBsz_n2 = min(floor(zBsz(2)), size(TMPvasc,2));
  TMPves = single(TMPvasc(floor((size(TMPvasc,1)-zBsz_n1)/2)+(1:zBsz_n1), ...
    floor((size(TMPvasc,2)-zBsz_n2)/2)+(1:zBsz_n2),zA+1:zB));
  szB = ceil(zBsz);
  TMPves_pad = zeros(szB(1), szB(2), szB(3), 'single');  % third dim uses szB(3)
  n1 = min(size(TMPves,1), szB(1)); n2 = min(size(TMPves,2), szB(2)); n3 = min(size(TMPves,3), szB(3));
  if n3 < szB(3)
    if exist('warn_append','file'), warn_append(lp0, 'PAD: TMPves z=%d < neur_ves_B z=%d, zero-padding %d layers\n', n3, szB(3), szB(3)-n3); else warning('simulate_optical_propagation:Z_PAD', 'PAD'); end
  end
  if n1 < szB(1) || n2 < szB(2)
    if exist('warn_append','file'), warn_append(lp0, 'PAD: TMPves [%d,%d] < neur_ves_B [%d,%d] (symmetric)\n', n1, n2, szB(1), szB(2)); else warning('simulate_optical_propagation:XY_PAD', 'PAD'); end
  end
  tx = floor((szB(1)-n1)/2); ty = floor((szB(2)-n2)/2); tz = floor((szB(3)-n3)/2);
  TMPves_pad(1+tx:tx+n1, 1+ty:ty+n2, 1+tz:tz+n3) = TMPves(1:n1, 1:n2, 1:n3);
  neur_ves_B = TMPves_pad + neur_ves_B.* ... 
...
colpx(1) = round(colpx(1)); colpx(2) = round(colpx(2));  % ensure integer indices
