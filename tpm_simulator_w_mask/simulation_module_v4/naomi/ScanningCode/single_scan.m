...
if z_sub == 1
    neur_vol2 = neur_vol;                                                  % No sub-sampling
end
...
if freq_opt
    ...
    ft_vol = fft(fft(neur_vol2, sz(1), 1), sz(2), 2);                     % FFT of volume
    if ~strcmp(class(ft_vol), class(psf2))                                % Ensure type match (single.*double not allowed)
        psf2 = cast(psf2, class(ft_vol));
    end
    scan_img = ifft(ifft(sum(ft_vol.*psf2,3), ...
...
%     scan_img = sum(scan_img, 3);                                           % Sum along the axial dimension
    y_ix     = floor((psf_sz(1)-1)/2) + [1,size(neur_vol2,1)];             % Get dim 1 size of image (same-conv crop; floor matches conv2(...,'same'))
    y_jx     = floor((psf_sz(2)-1)/2) + [1,size(neur_vol2,2)];             % Get dim 2 sizes of image (same-conv crop; floor matches conv2(...,'same'))
