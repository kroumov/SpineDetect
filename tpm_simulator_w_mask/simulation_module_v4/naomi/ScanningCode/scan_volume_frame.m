...
sfrac     = double(scan_params.sfrac);                                     % Subsampling factor (ensure double to avoid int64 from Python)
...
if(nargin<4)
  ...
  % Disabled: z_off random jitter for deterministic single-frame degradation
  % zmaxdiff = 2;
...
z_loc = max(1, floor(0.5*(N3-Np3))+z_off);  % ensure >=1 when N3==Np3 (padded slab)
...
TMPvol = zeros(N1,N2,N3,'single');                                 % Reset the temporary volume - It's an order of magnitude faster to scrap and start over

% array_SubSubTest/array_SubModTest: MEX in-place; .m returns result for fallback
for ll = 1:size(soma_act,1)
  if(soma_act(ll)>0)&&(~isempty(scan_vol.somaVol{ll,2}))
    r = array_SubSubTest(TMPvol,scan_vol.somaVol{ll,1},...
      scan_vol.somaVol{ll,2},soma_act(ll));
    if ~isempty(r), TMPvol = r; end
  end
end
...
if(isfield(scan_params,'nuc_label'))&&(scan_params.nuc_label>=1);
  for ll = 1:size(neur_vol.gp_nuc,1)
    if(nuc_act(ll)>0)&&(~isempty(scan_vol.nucVol{ll,2}))
      r = array_SubSubTest(TMPvol,scan_vol.nucVol{ll,1},...
...
for ll = 1:size(dend_act,1)                                        % For large volumes, mex functions decrease runtimes a lot
  if (dend_act(ll)>0)&&(~isempty(scan_vol.dendVol{ll,1}))
    r = array_SubSubTest(TMPvol,scan_vol.dendVol{ll,1},...
...
for ll = 1:size(bg_act,1)
  if(~isempty(scan_vol.axonVol{ll,1}) && bg_act(ll)>0)
    r = array_SubModTest(TMPvol, scan_vol.axonVol{ll,1}, ...
      scan_vol.axonVol{ll,2},bg_act(ll)); 
    if ~isempty(r), TMPvol = r; end
  end
end

% Ensure double scalar * double to avoid "integer" type error when sfrac from Python is int64
clean_img = (1.0/(2*double(sfrac)^2))*double(single_scan(TMPvol(:,:,...
...
if(~isempty(scan_vol.psfT))
  % Use actual convmask size (psfT may differ from main PSF Np1,Np2)
  psf_sz_xy = [size(scan_vol.psfT.convmask, 1), size(scan_vol.psfT.convmask, 2)];
  if(isempty(1:z_loc-1))
    top_img = blurredBackComp2(TMPvol,1:size(TMPvol,3),...
      scan_vol.psfT.freq_psf,scan_vol.psfT.weight, scan_vol.top_mask, 1, [], psf_sz_xy);
  else
    top_img = blurredBackComp2(TMPvol,1:z_loc-1,scan_vol.psfT.freq_psf,...
      scan_vol.psfT.weight, scan_vol.top_mask, 1, scan_vol.psfT.psfZ, psf_sz_xy);
  end
  if(isempty(z_loc+Np3:size(TMPvol,3)))
    bot_img = blurredBackComp2(TMPvol,1:size(TMPvol,3),...
      scan_vol.psfB.freq_psf,scan_vol.psfB.weight, scan_vol.bot_mask, 1, [], psf_sz_xy);
  else
    bot_img = blurredBackComp2(TMPvol,z_loc+Np3:size(TMPvol,3),...
      scan_vol.psfB.freq_psf,scan_vol.psfB.weight, scan_vol.bot_mask, 1, scan_vol.psfB.psfZ, psf_sz_xy);
  end
