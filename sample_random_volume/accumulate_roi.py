...
def accumulate_roi(input_path):
    ...
    all_coords = [item[:3] for items in db.values() for item in items]
    grid_size = max(c[0] for c in all_coords) + 1
    vol_shape = (grid_size * block_px, grid_size * block_px, grid_size * block_px)

    print(f"Nucleus: {nucleus_id} | Grid: {grid_size}^3 | Resolution: {res_nm} nm")

    # Stitch each channel and write TIFF
