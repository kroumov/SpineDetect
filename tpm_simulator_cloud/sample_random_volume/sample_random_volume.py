...
# Ensure non-degenerate interval for cloudfiles IntervalTree (start_us < end_us)
...
def build_whitelist_with_stats(client, table_names, table_filters=None):
    """Build whitelist from tables. No valid filter applied."""
    log = _get_logger()
    t0 = time.perf_counter()
...
if __name__ == "__main__":
    ...
    def _on_sigterm(signum, frame):
        print("\nSignal received. Exiting.", flush=True)
        os._exit(143)
    ...
    try:
        ...
        if failed > 0:
            print(f"WARN: {failed}/{len(tasks)} blocks failed (check log for details)", flush=True)
