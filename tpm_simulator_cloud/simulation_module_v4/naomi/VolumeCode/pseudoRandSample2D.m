...
if(nargin<5);   pdf    = ones(sz,'single'); end
...
[X,Y] = meshgrid(-ceil(2*width):ceil(2*width));                             % Set up a mesh-grid
gpdf  = single(-weight*exp(-(X.^2+Y.^2)/(width^2)));                        % Greate a Gaussian PDF (proportional to)
pos   = zeros(nsamps,2);                                                    % Initialize the position matrix
i     = 1;
numit = 0;
...
function [xc,xi] = correctOffsetRoundError(xc,xi);

if diff(xc) > diff(xi)
    xc(1) = xc(1) + (diff(xc)-diff(xi));
elseif diff(xc) < diff(xi)
    xi(1) = xi(1) + (diff(xi)-diff(xc));
else
