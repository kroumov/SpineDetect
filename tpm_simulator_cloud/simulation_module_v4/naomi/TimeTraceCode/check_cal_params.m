...
switch lower(prot_type)
    case 'voltage'
        cal_params.vPars       = spk_calcium('par');
        cal_params.vPars.T     = [];     % second, total length in second
        cal_params.vPars.a     = 0.2;    % df/f, not times
        cal_params.vPars.tau   = 0.02;   % decay time constrain
        cal_params.vPars.dt    = .00125; % 800Hz acquisition
        cal_params.vPars.sigma = 0.1;    % noise level estimator
    otherwise
end
