...
opts = struct();
opts.fluor_scale     = 0.8;   % fluorescence scaling (0–1)
...
opts.psf_sz          = [4, 4, 12];  % PSF size [x,y,z] in µm
opts.neuropil_enable = true;  % add neuropil/background fluorescence (uses NAOMi masked_3DGP)
opts.neuropil_scale  = 0.08;  % neuropil max as fraction of neur_vol max (0.05–0.15 typical)
opts.neuropil_thresh = 0.05;  % background = voxels below this fraction of max
