...
[vcpx, ~, ~] = compute_vcpx(voxel_um, vol_depth_um, size(neurons,1), size(neurons,2), slab_z, ...
  get_opt(opts, 'psf_sz', []), get_opt(opts, 'opt_type', 'standard'), get_opt(opts, 'condition', 'standard'), log_path);
slab_vcpx = vcpx;
vcpx_xy = vcpx(1);
...
psf_sz = get_opt(opts, 'psf_sz', [4, 4, 12]);
if numel(psf_sz) < 3, psf_sz = [psf_sz(1), psf_sz(min(2,end)), 12]; end
...
% Effective NAOMi bridge overrides (naomi_bridge.m hardcoded)
taillength_eff = min(50, max(8, vol_sz_um(3)));
sampling_eff = min(50, max(2, vol_sz_um(1)/4));
zernike_str = '[0 0 0 0 0.03 0 0 0 0 0 0.04]';
...
naomi_section = sprintf([
  ...
  taillength_eff, sampling_eff, zernike_str, ...
  pad_xy, get_opt(opts, 'scan_avg', 1), get_opt(opts, 'sfrac', 1), ...
  fluor_scale, ...
