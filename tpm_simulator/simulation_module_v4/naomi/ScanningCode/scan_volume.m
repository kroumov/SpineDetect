...
%%% VOLUME SIZES                         
noise_params.sigscale = tpmSignalscale(tpm_params)*(spike_opts.dt)* ...
                                          (scan_params.sfrac^2)/(250*250); % Set the signal scaling based on the power levels and spatio-temporal volume (i.e. dwell time X voxel volume)
...
if(nargout > 0 && scan_params.movout)
    mov = zeros(floor([(N1)/sfrac, (N2)/sfrac, Nt]) - ...
                               floor(2*[scan_buff,scan_buff,0]/sfrac), 'single'); % Initialize movie array
end
...
if ~isstruct(PSF)
    for kk = 1:Nt                                                          % Iterate over time steps
        ...
        if(~isempty(psfT))
            ...
            psf_sz_xy = [size(psfT.convmask,1), size(psfT.convmask,2)];
            if(isempty(1:z_loc-1))
                top_img = blurredBackComp2(TMPvol,1:size(TMPvol,3),...
                           psfT.freq_psf,psfT.weight, top_mask, 1, [], psf_sz_xy, f0vol);          
            else
                top_img = blurredBackComp2(TMPvol,1:z_loc-1,psfT.freq_psf,...
                                         psfT.weight, top_mask, 1, psfT.psfZ, psf_sz_xy, f0vol);
            end
            if(isempty(z_loc+Np3:size(TMPvol,3)))
                bot_img = blurredBackComp2(TMPvol,1:size(TMPvol,3),...
                           psfB.freq_psf,psfB.weight, bot_mask, 1, [], psf_sz_xy, f0vol);                      
            else
                bot_img = blurredBackComp2(TMPvol,z_loc+Np3:size(TMPvol,3),...
                           psfB.freq_psf,psfB.weight, bot_mask, 1, psfB.psfZ, psf_sz_xy, f0vol);            
            end
...
%             movRaw = cat(3, movRaw, clean_img);                            % Store the raw image (no noise) if requested
        end
        if (nargout > 0 && scan_params.movout)
            mov(:,:,kk) = samp_img;    
%             mov = cat(3, mov, samp_img);                                   % Store the sampled image (with noise)
        end
        if (~isempty(scan_params.fsimPath))
